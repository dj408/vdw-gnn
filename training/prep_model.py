...
from models.escgnn_layer import ESCGNNLayer
from models.escgnn_modular import ESCGNNModular
from models.escgnn_radial import ESCGNNRadial
from models.escgnn_molecule import ESCGNN_MOLECULE
from models.escgnn_macaque import ESCGNN_macaque
from models.escgnn_macaque_supcon import ESCGNN_macaque_supcon
from models.escgnn_macaque_supcon_2 import ESCGNN_macaque_supcon_2
from models.custom_loss_fns import SupConCrossEntropyLoss
...
def _prepare_escgnn_radial_model(
    ...
    # Handle custom diffusion scales specified in the dataset config
    custom_scalar_scales = process_wavelet_scales_type(model_config.scalar_diffusion_scales)
    custom_vector_scales = None if ablate_vector_track else process_wavelet_scales_type(model_config.vector_diffusion_scales)
    ...
    model = ESCGNNRadial(
        ...
        vector_gate_mlp_nonlin_fn=MLP_NONLIN_MODULE_MAP[model_config.vector_gate_mlp_nonlin],
...
def prepare_escgnn_model(
    ...
    elif model_key == 'escgnn_layer':
        return _prepare_escgnn_layer_model(config, dataloader_dict, acc)
    elif model_key == 'escgnn_molecule':
        return _prepare_escgnn_molecule_model(config, dataloader_dict, acc)
    elif model_key == 'escgnn_macaque':
        return _prepare_escgnn_macaque_model(config, dataloader_dict, acc)
    elif model_key == 'escgnn_macaque_supcon':
        return _prepare_escgnn_macaque_supcon_model(config, dataloader_dict, acc)
    elif model_key == 'escgnn_supcon_2':
        return _prepare_escgnn_macaque_supcon_2_model(config, dataloader_dict, acc)
    else:
        raise ValueError(
            f"Unsupported model_key for prepare_escgnn_model: {model_key}. "
            "Supported values are 'escgnn', 'escgnn_radial', 'escgnn_modular', "
            "'escgnn_layer', and 'escgnn_molecule'."
        )


def _prepare_escgnn_layer_model(
    config: TrainingConfig,
    dataloader_dict: Dict[str, PyGDataLoader],
    acc: Optional[Accelerator] = None,
) -> Tuple[
    ...
    """
    Prepare the lightweight vector-only ESCGNN layer stack (ScatterMLP).
    """
    model_config = config.model_config
    dataset_config = config.dataset_config
...
def _prepare_escgnn_macaque_model(
    config: TrainingConfig,
    dataloader_dict: Dict[str, PyGDataLoader],
    acc: Optional[Accelerator] = None
) -> Tuple[
    ...
    """
    Prepare ESCGNN_macaque model: vector scattering from spike_data, invariants + time scalar → MLP.
...
def _prepare_escgnn_macaque_supcon_model(
    config: TrainingConfig,
    dataloader_dict: Dict[str, PyGDataLoader],
    acc: Optional[Accelerator] = None
) -> Tuple[
    ...
    """
    Prepare ESCGNN_macaque_supcon model: supervised contrastive learning for trajectory embeddings.
    Supports two modes: 'pretrain' (SupConLoss) and 'finetune' (frozen encoder + classifier).
    """
    model_config = config.model_config
    dataset_config = config.dataset_config

    # Helper to resolve activation strings to nn.Module classes
    def _resolve_activation(name: str, default_cls: type[nn.Module] = nn.ReLU) -> type[nn.Module]:
        key = str(name).lower()
        return MLP_NONLIN_MODULE_MAP.get(key, default_cls)

    # Get mode from model_config (case-insensitive for robustness)
    mode = str(getattr(model_config, 'mode', 'pretrain')).lower()
    freeze_encoder = getattr(model_config, 'freeze_encoder', False)
    temperature = getattr(model_config, 'temperature', 0.1)

    # For contrastive learning, task should be 'graph_multi_classification'
    task = dataset_config.task.lower()
    # is_classification = ('multi' in task) and ('class' in task)
    
    # if not is_classification:
    #     raise ValueError(
    #         f"ESCGNN_macaque_supcon requires task='graph_multi_classification', "
    #         f"got task='{task}'"
    #     )
    ...
    if mode == 'train':
        base_module_kwargs['loss_fn'] = SupConCrossEntropyLoss(temperature=temperature)
    
    # Classification setup: 7 conditions
    num_classes = 7
    base_module_kwargs['target_name'] = 'condition_idx'
    base_module_kwargs['metrics_kwargs']['num_classes'] = num_classes

    # Vector track kwargs
    vector_feat_key = getattr(dataset_config, 'vector_feat_key', 'spike_data')
    vector_operator_key = getattr(model_config, 'vector_operator_key', 'Q')
    vector_dim = getattr(dataset_config, 'vector_feat_dim', 6)
    
    # Diffusion kwargs for (vector-track) scattering
    diffusion_kwargs = {
        'scales_type': model_config.wavelet_scales_type,
        'J': getattr(model_config, 'J', 3),
        'include_lowpass': model_config.include_lowpass_wavelet,
        'diffusion_scales': torch.tensor(model_config.vector_diffusion_scales) \
            if model_config.vector_diffusion_scales != 'dyadic' else 'dyadic',
    }
    ...
    vector_track_kwargs = {
        'feature_key': vector_feat_key,
        'diffusion_op_key': vector_operator_key,
        'vector_dim': vector_dim,
        'num_layers': model_config.num_scattering_layers_vector,
        'diffusion_kwargs': diffusion_kwargs,
    }
    
    # VectorBatchNorm kwargs
    vector_bn_kwargs = {
        'momentum': getattr(model_config, 'vector_bn_momentum', 0.1),
        'eps': getattr(model_config, 'vector_bn_eps', 1e-6),
        'track_running_stats': getattr(model_config, 'vector_bn_track_running_stats', True),
    }
    
    # TemporalEncoder kwargs
    # def _parse_activation(act_str: str) -> type[nn.Module]:
    #     """Parse activation function string to class."""
    #     act_map = {
    #         'relu': nn.ReLU,
    #         'silu': nn.SiLU,
    #         'gelu': nn.GELU,
    #         'tanh': nn.Tanh,
    #     }
    #     return act_map.get(act_str.lower(), nn.ReLU)
    
    temporal_encoder_kwargs = {
        'hidden_channels': getattr(model_config, 'temporal_hidden_channels', [128, 128]),
        'kernel_sizes': getattr(model_config, 'temporal_kernel_sizes', [3, 5]),
        'paddings': getattr(model_config, 'temporal_paddings', None),
        'activation': _resolve_activation(getattr(model_config, 'temporal_activation', 'ReLU')),
        'out_dim': getattr(model_config, 'temporal_out_dim', 128),
    }

    # Projection hidden dim may be provided as a scalar or list; coerce to int
    proj_hidden_cfg = model_config.projection_hidden_dim
    if isinstance(proj_hidden_cfg, (list, tuple)):
        proj_hidden_dim = int(proj_hidden_cfg[0]) if len(proj_hidden_cfg) > 0 else 128
    else:
        proj_hidden_dim = int(proj_hidden_cfg)
    
    # ProjectionMLP kwargs
    projection_mlp_kwargs = {
        'hidden_dim': proj_hidden_dim,
        'embedding_dim': model_config.projection_embedding_dim,
        'activation': _resolve_activation(getattr(model_config, 'projection_activation', 'ReLU')),
        'use_batch_norm': getattr(model_config, 'projection_use_batch_norm', True),
    }
    
    # Classification hidden dims may be provided as scalar; ensure list
    class_hidden_cfg = model_config.classification_hidden_dims
    if isinstance(class_hidden_cfg, (int, float)):
        class_hidden_dims = [int(class_hidden_cfg)]
    else:
        class_hidden_dims = list(class_hidden_cfg)

    # ClassificationHead kwargs (for finetune/train modes)
    classification_head_kwargs = {
        'hidden_dims': class_hidden_dims,
        'activation': _resolve_activation(getattr(model_config, 'classification_activation', 'ReLU')),
        'dropout_p': getattr(model_config, 'classification_dropout_p', 0.1),
    }
    
    model = ESCGNN_macaque_supcon(
        base_module_kwargs=base_module_kwargs,
        vector_track_kwargs=vector_track_kwargs,
        vector_bn_kwargs=vector_bn_kwargs,
        temporal_encoder_kwargs=temporal_encoder_kwargs,
        projection_mlp_kwargs=projection_mlp_kwargs,
...
def _prepare_escgnn_macaque_supcon_2_model(
    config: TrainingConfig,
    dataloader_dict: Dict[str, PyGDataLoader],
    acc: Optional[Accelerator] = None
) -> Tuple[
...
# ------------------------------------------------------------
#           ESCGNN_MOLECULE PREPARATION FUNCTION
# ------------------------------------------------------------
def _prepare_escgnn_molecule_model(
    config: TrainingConfig,
    dataloader_dict: Dict[str, PyGDataLoader],
    acc: Optional[Accelerator] = None
) -> Tuple[
    ...
    base_module_kwargs = _setup_base_module_kwargs(
        acc.device, config, dataset_config
    )
    base_module_kwargs['has_lazy_parameter_initialization'] = False

    # Determine effective wavelet channel counts (0th + 1st + optional 2nd order)
    # First-order counts per track
    if model_config.wavelet_scales_type == 'custom':
        W1_vec = get_wavelet_count_from_scales(
            model_config.vector_diffusion_scales,
            model_config.include_lowpass_wavelet,
        )
        W1_line = get_wavelet_count_from_scales(
            model_config.scalar_diffusion_scales,
            model_config.include_lowpass_wavelet,
        )
    else:
        # Dyadic: treat first-order count as J + 1 when including lowpass
        W1_vec = int(model_config.J_vector) + 1
        W1_line = int(model_config.J_scalar_line) + 1

    num_orders_vector = int(model_config.num_scattering_layers_vector)
    num_orders_scalar = int(model_config.num_scattering_layers_scalar)
    W2_vec = (W1_vec * (W1_vec - 1)) // 2 if num_orders_vector > 1 else 0
    W2_line = (W1_line * (W1_line - 1)) // 2 if num_orders_scalar > 1 else 0

    # Total channels along wavelet axis per track
    num_wavelets_vector = int(1 + W1_vec + W2_vec)
    num_wavelets_scalar = int(1 + W1_line + W2_line)
    print(f"[DEBUG] num_wavelets_scalar (0/1/2/total): 1/{W1_vec}/{W2_vec}/{num_wavelets_vector}")
    print(f"[DEBUG] num_wavelets_vector (0/1/2/total): 1/{W1_line}/{W2_line}/{num_wavelets_scalar}")

    # Compute base edge feature width C0, then scattered feature width C0 * W_total
    include_l2 = bool(model_config.include_l2_distance_edge_feat)
    include_bonds = bool(model_config.include_bond_attr_edge_feat)
    num_rbf_scatter = int(model_config.num_rbf_scatter)
    num_bond_types = int(dataset_config.num_bond_types) if include_bonds else 0
    C0_edge = (1 if include_l2 else 0) + num_rbf_scatter + num_bond_types
    edge_context_dim = int(C0_edge * num_wavelets_scalar)

    readout_kwargs = {
        "readout_type": model_config.readout_type,  # "mlp" | "deepsets"
        "hidden_dim": model_config.mlp_hidden_dim \
            if model_config.readout_type == "mlp" \
            else model_config.deepsets_hidden_dim,
        "dropout": model_config.mlp_dropout_p,
        "output_dim": dataset_config.target_dim,
        "activation": model_config.mlp_nonlin,
    }

    model = ESCGNN_MOLECULE(
        edge_context_dim=edge_context_dim,
        num_atom_types=int(dataset_config.num_atom_types),
        atomic_number_key=dataset_config.atomic_number_attrib_key,
        num_wavelets_vector=num_wavelets_vector,
        num_wavelets_scalar=num_wavelets_scalar,
        vector_scatter_feature_key=dataset_config.vector_scatter_feature_key,
        line_scatter_feature_key=dataset_config.line_scatter_feature_key,
        num_msg_pass_layers=model_config.msg_pass_num_layers,
        base_module_kwargs=base_module_kwargs,
        readout_kwargs=readout_kwargs,
    )
...
,
        ...
        if ds_is_wind:
            ...
            if isinstance(vec_key, str) and len(vec_key) > 0:
                setattr(pyg_model, 'pos_input_key', vec_key)
        # Some datasets (e.g., wind) use edge_weight as normalized adjacency weights,
        # not geometric distances. Let TFN recompute lengths from positions instead.
        setattr(pyg_model, "ignore_edge_weight", True)
    elif model_key == 'legs':
