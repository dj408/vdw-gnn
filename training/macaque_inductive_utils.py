"""
Utilities shared by inductive macaque pipelines (MARBLE + ESCGNN SupCon v2).
...
def build_train_graph(
    ...
    """
    Construct a PyG Data object for MARBLE/ESCGNN training using train nodes only.
...
def _maybe_save_fold_embeddings(
    ...
    model_for_eval = trained_model.module if hasattr(trained_model, "module") else trained_model
    mode_attr = getattr(model_for_eval, "mode", None)
    if not (
        isinstance(model_for_eval, ESCGNN_macaque_supcon)
        and isinstance(mode_attr, str)
        and (mode_attr.lower() == "pretrain")
    ):
        return
