...
def load_dataset(
    ...
    # If using HDF5 for operators, return ESCGNNDatasetHDF5
    if hasattr(data_config, 'h5_path') and data_config.h5_path:
        ...
        with h5py.File(data_config.h5_path, 'r') as _h5f:
            ...
            # New: include line-graph operator when present and requested
            try:
                if (config.model_config.model_key in ('escgnn_molecule',)):
                    lkey = getattr(config.model_config, 'line_operator_key', 'P_line')
                    if lkey in _h5f:
                        op_keys = op_keys + (lkey,)
            except Exception:
                pass
