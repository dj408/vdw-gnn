...
Required:
  --model=MODEL        One of: cebra, marble, escgnn, lfads
...
Optional:
  ...
  --model_config=PATH  Model YAML to merge (ESC GNN only; default: config/yaml_files/macaque/escgnn_supcon_2.yaml)
...
if [[ "$MODEL_LOWER" != "cebra" && "$MODEL_LOWER" != "marble" && "$MODEL_LOWER" != "escgnn" && "$MODEL_LOWER" != "lfads" ]]; then
  echo "Error: --model must be one of cebra|marble|escgnn|lfads"
  exit 1
fi
