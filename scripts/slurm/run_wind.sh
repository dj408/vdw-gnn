...
#SBATCH --output=/bsuhome/davejohnson408/scratch/escgnn/job_outputs/%j

# Example CLI calls of this script: 
# NOTES: 
# - use 'experiment.yaml' for --model dd-tnn 
# set --dataset=wind_rot for wind rotation dataset

# --- wind ---
# sbatch scripts/slurm/run_wind.sh --dataset wind --replications 5 --knn_k 3 --local_pca_k 10 --sample_n 2000 --mask_prop 0.3 --do_rotation_eval --model escgnn_layer --config escgnn_layer.yaml --exp_name escgnn_layer


# sbatch scripts/slurm/run_wind.sh --dataset wind --replications 5 --knn_k 3 --local_pca_k 10 --sample_n 100,200,300,400 --mask_prop 0.1,0.3,0.5 --do_rotation_eval --model escgnn --config escgnn.yaml --exp_name escgnn


# sbatch scripts/slurm/run_wind.sh --dataset wind --replications 5 --knn_k 3 --sample_n 100,200,300,400 --mask_prop 0.1,0.3,0.5 --do_rotation_eval --model dd-tnn --config experiment.yaml --exp_name tnn

# sbatch scripts/slurm/run_wind.sh --model dd-tnn --dataset wind --config experiment.yaml --replications 5 --sample_n 100,200,300,400 --mask_prop 0.1,0.3,0.5 --exp_name tnn_0

# --- wind_rot ---
# sbatch scripts/slurm/run_wind.sh --dataset wind_rot --replications 5 --knn_k 3 --local_pca_k 10 --sample_n 100,200,300,400 --mask_prop 0.1,0.3,0.5 --exp_name escgnn --model escgnn --config escgnn.yaml

ROOT_DIR="/bsuscratch/davejohnson408/escgnn"
CONDA_ENV="torch-env3"
PYTHON_BIN="python3"

show_help() {
  echo "Usage: sbatch $0 --model escgnn|escgnn_layer|dd-tnn|gcn|gat|gin|legs|egnn|tfn --sample_n LIST --mask_prop LIST [--root_dir PATH] [--conda_env ENV] [script args...]"
  echo ""
  echo "Runs run_wind_experiments.py (escgnn/escgnn_layer/gcn/gat/gin/legs/egnn/tfn) or run_wind_tnn.py (dd-tnn)."
  ...
  echo ""
  echo "Examples:"
  echo "  sbatch $0 --model escgnn \\"
  echo "    --root_dir /bsuscratch/davejohnson408/escgnn \\"
  echo "    --config code/config/yaml_files/wind/escgnn.yaml \\"
...
if [[ -z "$MODEL" ]]; then
  echo "Error: --model is required (escgnn|escgnn_layer|dd-tnn|gcn|gat|gin|legs|egnn|tfn)."
  show_help
  exit 1
fi

if [[ "$MODEL" != "escgnn" && "$MODEL" != "escgnn_layer" && "$MODEL" != "dd-tnn" && "$MODEL" != "gcn" && "$MODEL" != "gat" && "$MODEL" != "gin" && "$MODEL" != "legs" && "$MODEL" != "egnn" && "$MODEL" != "tfn" ]]; then
  echo "Error: --model must be one of 'escgnn', 'escgnn_layer', 'dd-tnn', 'gcn', 'gat', 'gin', 'legs', 'egnn', 'tfn'."
  show_help
  exit 1
fi
...
# passes just the filename (e.g., 'escgnn.yaml').
...
if [[ "$MODEL" == "escgnn" || "$MODEL" == "escgnn_layer" || "$MODEL" == "gcn" || "$MODEL" == "gat" || "$MODEL" == "gin" || "$MODEL" == "legs" || "$MODEL" == "egnn" || "$MODEL" == "tfn" ]]; then
