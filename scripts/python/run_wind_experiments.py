...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", type=Path, default=Path("config/yaml_files/wind/escgnn.yaml"))
...
def attach_pq(
    ...
    Keeps the data as an ESCGNNData object with operator_keys=('P', 'Q').
    """
    # Remove pre-existing operator_keys to avoid duplicate kwargs when constructing ESCGNNData
...
def run_replication(
    ...
    if "escgnn" in model_key:  # includes escgnn_layer
    ...
    if do_rotation_eval and base_data_for_rotation is not None:
        ...
        if debug and ("escgnn" in model_key):
...
def main():
    ...
    exp_name = args.exp_name or getattr(base_config.model_config, "model_key", "escgnn")
