...
special support for the ESCGNN-family models and comparisons.
"""
import os
import shutil
import sys
...
def run_one_fold(
    ...
    if 'escgnn' in key_lower:
        # ESCGNN-family models (escgnn, escgnn_radial, escgnn_modular, etc.)
    ...
    if (
        config.model_config.model_key == 'escgnn_supcon_2'
...
def run_evaluation(
    ...
    # Special case: ESCGNN_macaque_supcon in 'pretrain' mode.
    # In this setting, the model learns embeddings without a trained classifier head.
    # For evaluation, run a multinomial logistic regression linear probe on the
    # learned embeddings and report its accuracy as a clustering-quality metric.
    model_for_type_check = trained_model.module if hasattr(trained_model, "module") else trained_model
    mode_attr = getattr(model_for_type_check, "mode", "")
    is_supcon_pretrain = isinstance(model_for_type_check, ESCGNN_macaque_supcon) \
        and isinstance(mode_attr, str) and (mode_attr.lower() == "pretrain")

    if is_supcon_pretrain:
        metric_scores_dict = _run_cluster_probes_on_embeddings(
            config=config,
            dataloader_dict=dataloader_dict,
            trained_model=trained_model,
            accelerator=accelerator,
            eval_metrics=eval_cluster_metrics,
            num_clusters=mk.get('num_classes', config.dataset_config.target_dim),
            metric_kwargs=eval_cluster_metric_kwargs,
        )
    else:
    ...
    if accelerator.is_main_process:
        ...
        # Select which metrics to print. For ESCGNN_macaque_supcon in 'pretrain'
        # mode we treat this as a clustering-style evaluation for printing.
        _task_for_printing = config.dataset_config.task
        try:
            _model_for_type_check = trained_model.module if hasattr(trained_model, "module") else trained_model
            _mode_attr = getattr(_model_for_type_check, "mode", "")
            if isinstance(_model_for_type_check, ESCGNN_macaque_supcon) \
            and isinstance(_mode_attr, str) \
            and (_mode_attr.lower() == "pretrain"):
                _task_for_printing = "clustering"
        except Exception:
            _task_for_printing = config.dataset_config.task
...
def main(clargs):
    ...
    if acc.is_main_process and config.snapshot_name is None:
        # Use a stable experiment directory name for ESCGNN-family models
        mk_lower = str(getattr(config.model_config, "model_key", "")).lower()
        if "escgnn" in mk_lower:
            model_slug = "escgnn"
        else:
    ...
    if config.experiment_type == 'tvt':  # 'train/valid/[test]' splits
        ...
        if config.config_save_path and acc.is_main_process:
            ...
            try:
                ...
                # Model-level YAML (e.g., 'escgnn_supcon.yaml')
    ...
    # ------------------------------------------------------------------
        ...
        if not is_macaque_kfolds:
            ...
            if acc.is_main_process:
                ...
                val_folds_is = range(1, config.dataset_config.k_folds) + [0]
        ...
        is_supcon2 = config.model_config.model_key == 'escgnn_supcon_2'
