#!/usr/bin/env python3
"""
Dispatch k-fold macaque runs per-day for CEBRA, MARBLE, ESCGNN, or LFADS.
...
- ESCGNN: merge experiment.yaml with model config (default escgnn_supcon_2.yaml)
...
def main() -> None:
    ...
    parser.add_argument("--model", required=True, choices=["cebra", "marble", "escgnn", "lfads"])
    parser.add_argument("--days", required=True, help="Day list like '0-3,7,10-12'.")
    parser.add_argument(
        "--root_dir",
        type=Path,
        required=True,
        help="Project root (e.g., /bsuscratch/.../escgnn).",
    )
    parser.add_argument(
        "--base_config",
        type=Path,
    ...
    parser.add_argument(
        "--model_config",
        type=Path,
        help="Optional model YAML to merge (used for ESCGNN).",
    )
    parser.add_argument(
        "--save_dir_base",
        type=Path,
    ...
    if model == "escgnn":
        default_model_cfg = code_dir / "config" / "yaml_files" / "macaque" / "escgnn_supcon_2.yaml"
    ...
    for day in days:
        ...
        if model == "cebra":
            ...
            cmd = ["python3", str(runner), "--config", str(day_cfg), "--day_idx", str(day)]
        elif model == "marble":
            runner = code_dir / "scripts" / "python" / "run_marble_macaque_inductive.py"
            cmd = ["python3", str(runner), "--config", str(day_cfg)]
        elif model == "lfads":
