#!/usr/bin/env python3
"""
End-to-end equivariance probe for ESCGNNModular on the 3D wind dataset.

Minimal usage:
  python3 tests/test_wind_escgnn_equivariance.py \
    --root_dir /bsuhome/davejohnson408/scratch/escgnn/ \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Probe rotation equivariance layer-by-layer for ESCGNNModular on wind."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("config/yaml_files/wind/escgnn.yaml"),
...
def main() -> None:
    ...
    # Build a single-graph dataloader for model preparation (reuses prepare_escgnn_model)
    loader = PyGDataLoader([data_a], batch_size=1, shuffle=False)
    dataloader_dict: Dict[str, PyGDataLoader] = {"train": loader}
    config, model, _ = prepare_escgnn_model(config, dataloader_dict, acc=acc)
