...
try:
    ...
    print("[ESCGNN] Warning: torch_scatter is not available. Some pooling operations will likely be slower.")
    torch_scatter = None
    _TORCH_SCATTER_AVAILABLE = False

            

class ESCGNN(bm.BaseModule):
    """
    ESCGNN (SO-equivariant LEarnable Scattering) model.
    ...
    def __init__(
        ...
        super(ESCGNN, self).__init__(**self.base_module_kwargs)
    ...
    def _node_pooling(
        ...
        def handle_pooling_exception(
            ...
            """
            msg = f"[ESCGNN][_pool_features] EXCEPTION in pooling type '{p_type}': {e}"
    ...
    def forward(self, batch: Batch | Data) -> Dict[str, torch.Tensor]:
        """Forward pass through the ESCGNN model."""
        ...
        if (self.mode in ('filter-combine', 'cross-track')):
            ...
            if (not self._mlp_initialized):
                ...
                if self.verbosity > 0:
                    print(f"[ESCGNN] MLP initialization complete")
    ...
    def _forward_cross_track(
        ...
        if not self._mlp_initialized:
            ...
            # MLP is now fully initialized
            if self.verbosity > 0:
                print(f"[ESCGNN] MLP initialization complete (cross-track)")
        ...
        *,
        ...
        if self.verbosity > 0:
            ...
            if init_vector and not self.ablate_vector_track:
                trk.append("vector")
            print(
                f"[ESCGNN] Initialized Wjxs BN for layer (tracks: {', '.join(trk)}) "
