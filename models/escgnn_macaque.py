"""
Simplified ESCGNN variant for macaque reaching: vector-only scattering from spike_data
...
class ESCGNN_macaque(ESCGNNModular):
    def __init__(
        self,
        *,
        ...
        """
        Initialize a macaque-specific ESCGNNModular subclass model.

        Args:
            base_module_kwargs: Passed to BaseModule (task, loss, metrics, device, etc.).
            vector_track_kwargs: Keys used by ESCGNNModular._scatter for the vector track:
            ...
            mixing_kwargs: Uses ESCGNNModular.MIXING_DEFAULTS; only vector mixer is used here.
            head_kwargs: Node MLP configuration. Reuses ESCGNNModular.HEAD_DEFAULTS keys:
