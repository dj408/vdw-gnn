"""
ESCGNNLayer: vector-only diffusion scattering + MLP block (wind setting).
...
For the wind dataset, this layer is configured via `config/yaml_files/wind/escgnn_layer.yaml`
...
class ESCGNNLayer(BaseModule):
    """
    Lightweight vector-only ESCGNN layer stack using diffusion scattering + MLP.
    ...
    def forward(
        ...
        # nnu.raise_if_nonfinite_tensor(x, name="escgnn_layer: x")
